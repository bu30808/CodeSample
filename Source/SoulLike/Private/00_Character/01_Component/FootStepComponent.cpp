...
bool UFootStepComponent::CreateFootStepTrace(FName SocketName, FHitResult& OutHit)
{
	...
	if (Owner.IsValid())
	{
		...
		FVector endLocation = Owner->GetMesh()->GetSocketLocation(SocketName) + (Owner->GetMesh()->GetUpVector() * TraceLength * -1); // 발 아래로 라인 트레이스
...
bool UFootStepComponent::CheckWaterDeep(FHitResult& WaterHit)
{
	if (Owner.IsValid())
	{
		...
		return UKismetSystemLibrary::SphereTraceSingle(this,startLocation,endLocation,10.f,UEngineTypes::ConvertToTraceType(ECC_Visibility),false,actorsToIgnore,EDrawDebugTrace::ForDuration,WaterHit,true,FColor::Blue,FColor::Silver,0.3f);
