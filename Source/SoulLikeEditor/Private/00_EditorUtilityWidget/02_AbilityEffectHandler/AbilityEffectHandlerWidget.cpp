...
void UAbilityEffectHandlerWidget::NativeConstruct()
{
	...
	if(CheckBox_Player)
	{
		CheckBox_Player->OnCheckStateChanged.AddUniqueDynamic(this,&UAbilityEffectHandlerWidget::OnPlayerCheckStateChanged);
	}
	if(CheckBox_Monster)
	{
		CheckBox_Monster->OnCheckStateChanged.AddUniqueDynamic(this,&UAbilityEffectHandlerWidget::OnMonsterCheckStateChanged);
	}
	if(CheckBox_Magic)
	{
		CheckBox_Magic->OnCheckStateChanged.AddUniqueDynamic(this,&UAbilityEffectHandlerWidget::OnMagicCheckStateChanged);
	}
	if(CheckBox_Physic)
	{
		CheckBox_Physic->OnCheckStateChanged.AddUniqueDynamic(this,&UAbilityEffectHandlerWidget::OnPhysicCheckStateChanged);
	}
	if(CheckBox_Cost)
	{
		CheckBox_Cost->OnCheckStateChanged.AddUniqueDynamic(this,&UAbilityEffectHandlerWidget::OnCostCheckStateChanged);
	}
	...
	{
		ComboBoxString_Effect->OnSelectionChanged.AddUniqueDynamic(this,&UAbilityEffectHandlerWidget::OnSelectionChanged);
	}
...
void UAbilityEffectHandlerWidget::OnSelectionChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
	if(ComboBoxString_Effect)
	{
		...
		{
			if(DetailsView_Effect->GetObject()==nullptr)
			{
				auto data = NewObject<UAbilityEffectHandler>();
				DetailsView_Effect->SetObject(data);
			}
			...
			if(AbilityEffects.Contains(SelectedItem))
			{
				...
				auto effect = AbilityEffects[SelectedItem].GetDefaultObject();
				auto data = Cast<UAbilityEffectHandler>(DetailsView_Effect->GetObject());
				data->AttackType = effect->AttackType;
				data->AttributeEffects = effect->AttributeEffects;
				data->ChainTag = effect->ChainTagFromAbility;
				data->bTriggerHitAnimation = effect->bTriggerHitAnimation;
				data->bUseChainSystem = effect->bUseChainSystemFromAbility;
				data->AttributeEffectsAffectedByOwnersAttribute = effect->AttributeEffectsAffectedByOwnersAttribute;
			}else
			{
